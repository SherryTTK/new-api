...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&SandboxOrganization{}, "SandboxOrganization"},
		{&SandboxOrgToken{}, "SandboxOrgToken"},
...
func migrateClickHouseLogDB() error {
	...
	// CREATE TABLE IF NOT EXISTS 不会给已有表补列，因此需要显式兼容二开的错误日志去重字段。
	if err := LOG_DB.Exec("ALTER TABLE logs ADD COLUMN IF NOT EXISTS error_resolved UInt8 DEFAULT 0").Error; err != nil {
		return err
	}
...
CREATE TABLE IF NOT EXISTS logs (
	...
	error_resolved UInt8 DEFAULT 0,
...
func migrateErrorResolvedField() {
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		migrateClickHouseErrorResolvedField()
		return
	}
...
func migrateClickHouseErrorResolvedField() {
	var needsMigration int64
	if err := LOG_DB.Model(&Log{}).
		Where("type = ? AND error_resolved = 0 AND request_id != ''", LogTypeError).
		Limit(1).Count(&needsMigration).Error; err != nil || needsMigration == 0 {
		return
	}
	...
	statements := []struct {
		...
		{
			query:     "ALTER TABLE logs UPDATE error_resolved = 1 WHERE type = ? AND error_resolved = 0 AND request_id != '' AND request_id IN (SELECT DISTINCT request_id FROM logs WHERE type = ? AND request_id != '') SETTINGS mutations_sync = 1",
			innerType: LogTypeConsume,
		},
		{
			query:     "ALTER TABLE logs UPDATE error_resolved = 1 WHERE type = ? AND error_resolved = 0 AND request_id != '' AND (request_id, created_at) NOT IN (SELECT request_id, MAX(created_at) FROM logs WHERE type = ? AND request_id != '' GROUP BY request_id) SETTINGS mutations_sync = 1",
			innerType: LogTypeError,
		},
	}
	for _, statement := range statements {
		if err := LOG_DB.Exec(statement.query, LogTypeError, statement.innerType).Error; err != nil {
			common.SysError("failed to migrate ClickHouse error_resolved: " + err.Error())
			return
		}
	}
	common.SysLog("ClickHouse error_resolved migration completed")
}
