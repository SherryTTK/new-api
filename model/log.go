...
func MaskErrorContentForUser(content string, requestId string) string {
	...
	if strings.HasPrefix(content, prefix) {
		statusCode := strings.TrimSpace(strings.SplitN(strings.TrimPrefix(content, prefix), ",", 2)[0])
		if requestId != "" {
...
func QueryUserConsumeLog(userId int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string) (logs []*Log, err error) {
	...
	err = tx.Order(order).Find(&logs).Error
	return logs, err
}
...
func GetSuccessLogByRequestId(requestId string, userId int) (*Log, error) {
	...
	if err := tx.Order(order).First(&log).Error; err != nil {
...
func RecordErrorLog(c *gin.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	...
	if requestId != "" && common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		// ClickHouse 日志没有自增 ID，先将同一请求的旧错误标记为已解决，再写入当前错误。
...
func resolveErrorLogs(requestId string) {
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		LOG_DB.Exec(
			"ALTER TABLE logs UPDATE error_resolved = 1 WHERE request_id = ? AND type = ? AND error_resolved = 0",
			requestId, LogTypeError,
		)
		return
	}
	LOG_DB.Model(&Log{}).
...
func GetLogsForDetailExport(startTimestamp int64, endTimestamp int64, tokenName string, userId int, username string) (logs []*Log, err error) {
	...
	if err = tx.Order(order).Find(&logs).Error; err != nil {
